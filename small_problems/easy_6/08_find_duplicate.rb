...
def find_dup(arr)
  arr_sorted = arr.sort
  arr_sorted.each_with_index do |elm, idx|
    return elm if elm == arr_sorted[idx + 1]
  end
end
...
# we could also use the Enumerable#find method with #count:

def find_dup2(arr)
  arr.find{ |elm| arr.count(elm) > 1 }
end

puts
p [
  find_dup2([1, 5, 3, 1]) == 1,
  find_dup2([18,  9, 36, 96, 31, 19, 54, 75, 42, 15,
            38, 25, 97, 92, 46, 69, 91, 59, 53, 27,
            14, 61, 90, 81,  8, 63, 95, 99, 30, 65,
            78, 76, 48, 16, 93, 77, 52, 49, 37, 29,
            89, 10, 84,  1, 47, 68, 12, 33, 86, 60,
