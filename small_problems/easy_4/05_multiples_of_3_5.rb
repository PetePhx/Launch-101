# Write a method that searches for all multiples of 3 or 5 that lie between 1
# and some other number, and then computes the sum of those multiples. For
# instance, if the supplied number is 20, the result should be 98
# (3 + 5 + 6 + 9 + 10 + 12 + 15 + 18 + 20).

# You may assume that the number passed in is an integer greater than 1.

def multisum num
  (1..num).select {|m| m % 3 == 0 || m % 5 == 0}.sum
end

# not wanting to use array, e.g. for very large numbers:
def multisum num
  sum = 0
  (1..num).each do |m|
    sum += m if m % 3 == 0 || m % 5 == 0
  end
  sum
end

p [multisum(3) == 3,
   multisum(5) == 8,
   multisum(10) == 33,
   multisum(1000) == 234168]

# Further Exploration

# Investigate Enumerable.inject (also known as Enumerable.reduce), How might
# this method be useful in solving this problem?

# Answer: the previous method implicitly used reduce/inject in the array sum
# method. This is equivalent to:

def multisum num
  (1..num).select {|m| m % 3 == 0 || m % 5 == 0}.reduce(:+)
end
puts
p [multisum(3) == 3,
   multisum(5) == 8,
   multisum(10) == 33,
   multisum(1000) == 234168]
